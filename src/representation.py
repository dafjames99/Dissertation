...
class UtilityClass(DotDict):
    def show_schema(self, obj = None, verbose = True):
        ...
        for key, value in obj.items() if isinstance(obj, dict) else obj.__dict__.items():
...
class DataLoader(UtilityClass):
    def __init__(self, text_variant: str, sentence_model_index: str):    
        self.job = DotDict()
        self.job.df = pd.read_csv(DATA_DICT['jobs'], index_col='job_id')
        self.job.df['fulltext'] = pd.read_csv(DATA_DICT['embeddings'][text_variant]['jobs_texts'])['jobs_texts']
        self.job.ids = self.job.df.index.to_numpy()
        self.job.dates = pd.to_datetime(self.job.df['date'])
        self.job.embedding = np.load(DATA_DICT['embeddings'][text_variant][f'jobs_embed_{sentence_model_index}'])
        
        self.repo = DotDict()
        self.repo.df = pd.read_csv(DATA_DICT['github']['repositories']['metadata'])
        self.repo.df['fulltext'] = pd.read_csv(DATA_DICT['embeddings'][text_variant]['repositories_texts'])['repositories_texts']
        self.repo.ids = self.repo.df.repository.to_numpy()
        self.repo.embedding = np.load(DATA_DICT['embeddings'][text_variant][f'repositories_embed_{sentence_model_index}'])
...
class SimilarityCalculator(UtilityClass):
    def __init__(self, dataloader: DataLoader = None, text_variant = None, sentence_model_index = None, **kwargs):
        if dataloader is None:
            try: 
                self.data = DataLoader(text_variant, sentence_model_index)
            except:
        ...
        self.df = pd.DataFrame(self.matrix, columns = self.data.job.ids, index = self.data.repo.ids)
    ...
    def n_repo_sort(self, job_id: int, n: int = 1, ascending = False, index_only = False, value_only = False) -> pd.Series:
        return self.df[job_id].sort_values(ascending = ascending).iloc[:n]
        
    def demonstrate(self, job_id: int, repo: Literal['highest', 'lowest'] | str | int = None):
...
class EmbeddingSimilarity:
    ...
    def __init__(self, sentence_model_index, text_variant):
        ...
        self.job_texts = pd.read_csv(DATA_DICT['embeddings'][self.text_variant]['jobs_texts'])['jobs_texts']
        self.repo_texts = pd.read_csv(DATA_DICT['embeddings'][self.text_variant]['repositories_texts'])['repositories_texts']
        ...
        self.job_embedding = np.load(DATA_DICT['embeddings'][text_variant][f'jobs_embed_{sentence_model_index}'])
        self.repo_embedding = np.load(DATA_DICT['embeddings'][text_variant][f'repositories_embed_{sentence_model_index}'])

        self.matrix = self.repo_embedding @ self.job_embedding.T
        self.df = pd.DataFrame(self.matrix, columns = self.job_id, index = self.repositories)
    ...
    def inspect_job_similarity(
        ...
        for repo in top_repos.index[:n_descriptions]:
            repo_desc = self.df_repos.loc[self.df_repos['repository'] == repo, 'description']
            desc_text = repo_desc.values[0] if not repo_desc.empty else "No description available"
            print(f"\n🔹 {repo}:\n{desc_text}")


    def candlestick(self, axis = 0, sort_by_mean = True, long = 20, small = 10, ax = None, grid = None, **kwargs):
        if ax is None:
            figsize = ((small, long) if kwargs.get('orientation') == 'horizontal' else (long, small))
            fig, ax = plt.subplots(figsize = figsize)
        
        
        grid = (grid if grid is not None else ('y' if kwargs.get('orientation') == 'horizontal' else 'x'))
        ax.grid(axis = grid, linestyle = '--', color = 'r', alpha = 0.5)
        
        if sort_by_mean:
            mean = self.df.mean(axis = (1 if axis == 0 else 0)).sort_values(ascending = False)
            indices = mean.index
        else:
            indices = (self.repositories if axis == 0 else self.job_id)        
        
        values = ([self.df.loc[i] for i in indices] if axis == 0 else [self.df[i] for i in indices])
        
        ax.boxplot(values, tick_labels = indices, **kwargs)
            
    def similarity_stats(self, axis = 0):
        values = ([self.df.loc[i] for i in self.repositories] if axis == 0 else [self.df[i] for i in self.job_id])
    ...
    def plot_stats(self, plot_type = 'hist', axis = 0, axs = None, ax_lab_add = None, **kwargs):
        ...
        if plot_type == 'hist':        
            ...
            ax.set_ylabel(f'{'Repositories' if axis == 0 else 'Jobs'} Standard Deviation')
            ax.set_title(f'{self.sentence_model} - {'Repositories' if axis == 0 else 'Jobs'} Statistics Histogram')

    @staticmethod
    def combine_scatters(*sims: "EmbeddingSimilarity", **kwargs):
        figsize = ((20, 10) if kwargs.get('figsize') is None else kwargs.get('figsize'))
        colors = (n_colors(len(sims), 'high_contrast') if kwargs.get('c') is None else kwargs.get('c'))
        fig, ax = plt.subplots(figsize = figsize)
    ...
    def compare_models(*sims: "EmbeddingSimilarity", **kwargs):
        ...
        fig.suptitle('Comparing Summary Statistics of Sentence Models\nSummarized Across Repositories')
        plt.show()
    ...
    def repo_stars_similarity(
        ...
        *methods,
        ...
        merged['stars_norm'] = (merged['stars'] - merged['stars'].min()) / (merged['stars'].max() - merged['stars'].min())

        # Fill missing similarities with 0
        for method in methods:
            merged[f"{method.__name__}_similarity"] = merged[f"{method.__name__}_similarity"].fillna(0)
        if drop_zero:
...
def plot_similarity_ribbon(df, period_col='Q', remove_zeros = True):
    ...
    ax.fill_between(x, mean - std, mean + std, color='blue', alpha=0.2, label='Mean ± Std Dev')
