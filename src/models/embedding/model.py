...
class TextVariant(ABC):
    ...
    def generate(self):
        ...
        self.process_and_save(
            "repositories", self.build_repos_texts, "repositories_texts"
        )


class TextV1(TextVariant):
    def build_jobs_texts(self):
        return (self.df_jobs["title"] + " " + self.df_jobs["description"]).tolist()
...
class TextV2(TextVariant):
    def build_jobs_texts(self):
        ...
        job_texts = (self.df_jobs["title"] + " " + self.df_jobs["description"]).tolist()
        return job_processor.batch_transform(
            job_texts, "BY_contains_topic_chunk", "BY_contains_pos", pos="PROPN"
        )

    def build_repos_texts(self):
        repo_processor = RepositoryTextProcessor(
            self.kwargs.get("noise_patterns"), lemmatize=self.kwargs.get("lemmatize")
        )
        repo_texts = (
            (
                self.df_repos["topics"].str.replace(",", " ")
                ...
                + self.df_repos["repo"]
                + " "
                + self.df_repos["description"].str.replace(
                    r"[^\x00-\x7F]+", "", regex=True
                )
                + " "
                + self.df_repos["readme"].apply(
...
class TextProcessor:
    ...
    def batch_transform(self, texts, *args, **kwargs):
        ...
        with tqdm(
            ...
            for doc in docs:
                out.append(self.transform(doc, *args, visualise=False, **kwargs)[0])
                pbar.update(1)
        return out
