...
# 4. Run the script: Run ```python preprocessing/preprocess_jobdata.py``` from the `src` directory

# ----- This will take a while, but will fully download the data from each source, and compile into a single file ------
...
class KaggleDownloader:
    ...
    def download_to_dir(self, handle: str, dir: Path, exist_ok: bool = True, unzip: bool = True) -> None:
...
class PreprocessJobsData:
    def __init__(self, source: str, config: dict, intermediate_dir: Path = INTERMEDIATE_DIR, raw_dir: Path = RAW_DIR):
    ...
    def merge(self):
        ...
        return reduce(lambda left, right: pd.merge(left, right, how=merge_config['how'], on=merge_config['on']), dfs)
    ...
    def json_to_df(self):
        ...
        with open(self.raw_dir / self.source / self.in_file, 'r') as f:
            for line in f:
                ...
                trimmed = {k: record[k] for k in self.config.get('desired_keys') if k in record}
                data.append(trimmed)
    ...
    def convert_dates(self):
        ...
        self.df['date'] = pd.to_datetime(self.df['date'], format = "%Y-%m-%d").dt.to_period('D')
    ...
    def create_final(select_columns: list[str], from_dir: Path, out_file: Path) -> None:
        ...
        mask = master_df['title'].str.contains(jtt.title_match_pattern, regex=True, na=False)
        master_df = master_df[mask].copy()
        
        master_df['clean_title'] = master_df['title'].apply(jtt.clean_title)
        master_df['mapped_title'] = master_df['clean_title'].apply(jtt.map_title_to_category)

        master_df.reset_index(inplace = True, drop = True)
        master_df['job_id'] = master_df.index

        master_df['description'] = master_df['description'].apply(lambda x: re.sub(r'\s+', ' ', x).strip() if isinstance(x, str) else '')
...
if __name__ == '__main__':
    ...
    for source, config in PREPROCESSING_CONFIG.items():
        ...
        obj = PreprocessJobsData(source, config, intermediate_dir=INTERMEDIATE_DIR, raw_dir = RAW_DIR)
